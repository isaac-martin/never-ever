...
function never_ever_assets_header() {
  ...
  $file = 'x-header';
  wp_enqueue_script( 'never_ever-header', get_stylesheet_directory_uri() . '/js/' . $file . '.js', $deps = array('jquery'), filemtime( get_template_directory() . '/js/' . $file . '.js' ), false );

  // Register and enqueue our main stylesheet with versioning based on last modified time
  wp_register_style( 'never_ever-style', get_stylesheet_uri(), $dependencies = array(), filemtime( get_template_directory() . '/style.css' ) );
  wp_enqueue_style( 'never_ever-style' );
  ...
  wp_register_script('scroll', "//cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.5/ScrollMagic.min.js", array('jquery'), '2.0.5'); // Scroll Magic
  wp_enqueue_script('scroll'); // Enqueue it!

  wp_register_script('gsap', "//cdnjs.cloudflare.com/ajax/libs/gsap/1.14.2/TweenMax.min.js", array('jquery'), '1.14.2'); // GSAP
  wp_enqueue_script('gsap'); // Enqueue it!

  wp_register_script('gsapsm', "//cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.5/plugins/animation.gsap.js", array('jquery'), '2.0.5'); // GSAP
  wp_enqueue_script('gsapsm'); // Enqueue it!




}
add_action( 'wp_enqueue_scripts', 'never_ever_assets_header' );



// Load footer assets; a more complex example of a smooth asset-loading approach for WordPress themes
function never_ever_assets_footer() {

  // Initialize variables
  $name = 'never_ever-footer';   // This is the script handle
  ...
  if ( never_ever_SCRIPTS_PAGELOAD && ( is_archive() || is_home() || is_search() ) ) {
...
add_action( 'wp_enqueue_scripts', 'never_ever_assets_singular' );
